...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
	...
	if((threadColId < width) && (threadRowId < height))
	{
		int offset_out 	= threadRowId * width;
		int offset 	= offset_out * 3;

		float *pixel = &image[offset + threadColId * 3];

		image_out[offset_out + threadColId] = pixel[0] * 0.0722f + //B
						      pixel[1] * 0.7152f + //G
						      pixel[2] * 0.2126f;  //R	
	}
	__syncthreads();
...
#elif SHMEx
                sh_block[idx + (threadIdx.y*2)] = image[offset_t];
                if(threadIdx.x == (blockDim.x - 1))
                {
                        sh_block[idx + (threadIdx.y*2) + 1] = image[offset_t + 1];
                        sh_block[idx + (threadIdx.y*2) + 2] = image[offset_t + 2];
                }
                if(threadIdx.y == (blockDim.y - 1))
                {
                        sh_block[idx + ((threadIdx.y + 1)*2) + blockDim.x] = image[(index_y + 1) * width + index_x];
                        sh_block[idx + ((threadIdx.y + 2)*2) + (2*blockDim.x)] = image[(index_y + 2) * width + index_x];
                }
                if((threadIdx.x == (blockDim.x - 1)) && (threadIdx.y == (blockDim.y - 1)))
                {
                        sh_block[idx + ((threadIdx.y + 1)*2) + blockDim.x + 1] = image[(index_y + 1) * width + index_x + 1];
                        sh_block[idx + ((threadIdx.y + 1)*2) + blockDim.x + 2] = image[(index_y + 1) * width + index_x + 2];
                        sh_block[idx + ((threadIdx.y + 2)*2) + (2*blockDim.x) + 1] = image[(index_y + 2) * width + index_x + 1];
                        sh_block[idx + ((threadIdx.y + 2)*2) + (2*blockDim.x) + 2] = image[(index_y + 2) * width + index_x + 2];
                }
                __syncthreads();
		
		 image_out[offset] = gpu_applyFilter(&sh_block[idx + (threadIdx.y*2)],
                                                blockDim.x+2, gaussian, 3);
#else
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
	...
	if (index_x < (width - 2) && index_y < (height - 2))
	{
		...
		float gx = gpu_applyFilter(&sh_block[idx], blockDim.x, sobel_x, 3);
                float gy = gpu_applyFilter(&sh_block[idx], blockDim.x, sobel_y, 3);
#elif SHMEx
		sh_block[idx + (threadIdx.y*2)] = image[offset_t];
		if(threadIdx.x == (blockDim.x - 1))
		{
		...
		float gx = gpu_applyFilter(&sh_block[idx + (threadIdx.y*2)], blockDim.x+2, sobel_x, 3);
                float gy = gpu_applyFilter(&sh_block[idx + (threadIdx.y*2)], blockDim.x+2, sobel_y, 3);
#else
